...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        Task expectedTaskRecurring =
                new TaskBuilder(TASK2).withTags().withRecurring(VALID_RECURRING, LocalDateTime.now()).build();
        assertParseSuccess(parser, NAME_DESC_TASK2 + PRIORITY_DESC_TASK2 + DESCRIPTION_DESC_TASK2 + RECURRING, new AddCommand(expectedTaskRecurring));

        // with recurring and reminder 
        Task expectedTaskRecurRem =
                new TaskBuilder(TASK2).withTags().withReminder(VALID_REMINDER).withRecurring(VALID_RECURRING, LocalDateTime.now()).build();
        assertParseSuccess(parser, NAME_DESC_TASK2 + PRIORITY_DESC_TASK2 + DESCRIPTION_DESC_TASK2 + REMINDER + RECURRING, new AddCommand(expectedTaskRecurRem));

        // with recurring, reminder and tags
        Task expectedTaskEverything =
                new TaskBuilder(TASK2).withTags(VALID_TAG_HELP, VALID_TAG_MA1521).withReminder(VALID_REMINDER).withRecurring(VALID_RECURRING, LocalDateTime.now()).build();
        assertParseSuccess(parser, NAME_DESC_TASK2 + PRIORITY_DESC_TASK2 + DESCRIPTION_DESC_TASK2  + TAG_DESC_MA1521
        + TAG_DESC_HELP + REMINDER + RECURRING, new AddCommand(expectedTaskEverything));
    }
