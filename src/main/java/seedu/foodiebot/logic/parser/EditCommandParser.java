...
public class EditCommandParser implements Parser<EditCommand> {
    ...
    public EditCommand parse(String args) throws ParseException {
        ...
        EditCanteenDescriptor editCanteenDescriptor = new EditCanteenDescriptor();
        if (argMultimap.getValue(PREFIX_NAME).isPresent()) {
            editCanteenDescriptor.setName(
                    ParserUtil.parseName(argMultimap.getValue(PREFIX_NAME).get()));
        }

        parseTagsForEdit(argMultimap.getAllValues(PREFIX_TAG))
                .ifPresent(editCanteenDescriptor::setTags);

        if (!editCanteenDescriptor.isAnyFieldEdited()) {
            throw new ParseException(EditCommand.MESSAGE_NOT_EDITED);
        }

        return new EditCommand(index, editCanteenDescriptor);
    }
