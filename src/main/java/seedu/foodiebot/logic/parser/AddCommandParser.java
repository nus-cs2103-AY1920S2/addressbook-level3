...
import static seedu.foodiebot.logic.parser.CliSyntax.PREFIX_NOOFSTALLS;
...
public class AddCommandParser implements Parser<AddCommand> {
    ...
    public AddCommand parse(String args) throws ParseException {
        ...
        int numberOfStalls = ParserUtil.parseNoOfStalls(argMultimap.getValue(PREFIX_NOOFSTALLS).get());
        Set<Tag> tagList = ParserUtil.parseTags(argMultimap.getAllValues(PREFIX_TAG));

        CanteenStub canteen = new CanteenStub(name, numberOfStalls, tagList);

        return new AddCommand(canteen);
    }
