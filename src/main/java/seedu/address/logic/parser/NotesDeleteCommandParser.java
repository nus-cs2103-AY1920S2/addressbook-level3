...
public class NotesDeleteCommandParser implements Parser<NotesDeleteCommand> {
    ...
    public NotesDeleteCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_NOTES_PATH, PREFIX_NOTES_PATH_TYPE, PREFIX_NOTES_FILE_NAME);

        if (!arePrefixesPresent(argMultimap, PREFIX_NOTES_PATH, PREFIX_NOTES_PATH_TYPE, PREFIX_NOTES_FILE_NAME)
                || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, NotesDeleteCommand.MESSAGE_USAGE));
        }
        ...
        Notes note = ParserUtil.parseNotesDeleteOperation(fullPathName,
                argMultimap.getValue(PREFIX_NOTES_PATH_TYPE).get());

        return new NotesDeleteCommand(note);
    }
