...
public class EditCourseCommandParser implements Parser<EditCourseCommand> {
    ...
    public EditCourseCommand parse(String args) throws ParseException {
        ...
        try {
            ...
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, EditCourseCommand.MESSAGE_USAGE), pe);
        }
        ...
        if (argMultimap.getValue(PREFIX_COURSEID).isPresent()) {
            editCourseDescriptor.setID(ParserUtil.parseCourseID(argMultimap.getValue(PREFIX_COURSEID).get()));
        }
