...
/**
 * A list of finances that enforces uniqueness between its elements and does not allow nulls.
 * A finance is considered unique by comparing using {@code Finance#isSameFinance(Finance)}. As such, adding and updating of
 * finances uses Finance#isSameFinance(Finance) for equality so as to ensure that the finance being added or updated is
 * unique in terms of identity in the UniqueFinanceList. However, the removal of a finance uses finance#equals(Object) so
 * as to ensure that the finance with exactly the same fields will be removed.
 *
...
public class UniqueFinanceList implements Iterable<Finance> {
     ...
     * The finance must not already exist in the list.
     */
    ...
    /**
     * Replaces the finance {@code target} in the list with {@code editedFinance}.
     * {@code target} must exist in the list.
     * The Finance identity of {@code editedFinance} must not be the same as another existing finance in the list.
     */
     ...
     * The finance must exist in the list.
     */
