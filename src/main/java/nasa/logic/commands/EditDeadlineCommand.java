...
public class EditDeadlineCommand extends Command {
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        List<Deadline> lastShownList = moduleUniqueDeadlineList.getDeadlineList();
        ...
        if (!deadlineToEdit.isSameDeadline(editedDeadline) && model.hasDeadline(moduleCode, editedDeadline)) {
            throw new CommandException(MESSAGE_DUPLICATE_DEADLINE);
        }

        model.setDeadlineByIndex(moduleCode, index, editedDeadline);
        model.updateFilteredModuleList(PREDICATE_SHOW_ALL_MODULES);
        model.updateFilteredDeadlineList(PREDICATE_SHOW_ALL_ACTIVITIES);
