...
module Slim::Helpers
  ...
  # a block that will run between the opening and closing offers.
  #
  ...
  # Returns formatted style/link and script offers for header.
  def styles_and_scripts
    ...
    styles.each do |item|
      if item.key?(:text)
        offers << html_tag(:style, {}, item[:text])
      else
        offers << html_tag(:link, rel: 'stylesheet', href: urlize(*item[:href]))
      end
    end

    scripts.each do |item|
      if item.key? :text
        offers << html_tag(:script, {type: item[:type]}, item[:text])
      else
        offers << html_tag(:script, type: item[:type], src: urlize(*item[:src]))
      end
    end
