...
** Highlights:
*** The `SuggestionEngine` also provides a command input line auto-fill feature. When the user presses the `↓` button to select a suggestion, followed by
...
*** When the user presses the `↓` button to select the suggestion followed by `Enter`, the note chosen will immediately be opened.
