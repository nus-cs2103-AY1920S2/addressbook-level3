...
*** *What it does:* +
Receipt is used to store multiple Entries, and these Entries are meant to store the information about a particular Item and the Persons involved in splitting the cost of that item.
*** *Justification:* +
...
*** *Highlights:* +
These commands are required to prevent an exception being thrown with regards to negative BigDecimal values, as explained in the text above.

. *Added CalculateUtil and ParserUtil for the Expense Splitter portion of EYLAH*
*** *What it does:* +
CalculateUtil does all the handling of calculations required in EYLAH. ParserUtil parses all the inputted texts and outputs them as the required Objects.
*** *Justification:* +
