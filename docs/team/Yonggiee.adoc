...
** Highlights: Many of the future features will require the exercise classes. Therefore, it was essential to have in-depth analysis on how the classes are implemented. I also wrote a substantial amount of code to test the classes to reduce further bugs when expanding the application.
...
** Highlights: This enhancement requires a good understanding of Javafx. I have gotten a good grasp at using it to provide GUI for java application.
- The exercise progession graph +
I used `LineChart` in Javafx to display the graph. The X axis to be shown is date but JavaFx only provides `NumberAxis` and `CategoryAxis`. To display the axis as date, I used the `NumberAxis` but also had to use `setTickLabelFormatter()` to change the label from `epochDay` to date.
- The client's information displayed after `view-c` command +
...
*** Wrote the implementation of exercise classes and commands in Developer Guide (.... pr)
*** Wrote the command guide for `list-c`, `add-e` and `edit-e` in User Guide (.... pr)
*** Changed `UI` and `Storage` diagrams of Developer Guide (.... pr)
** Community:
