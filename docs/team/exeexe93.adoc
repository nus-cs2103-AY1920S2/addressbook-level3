...
Considerations have to be made to differentiate the data for `Order` and `ReturnOrder`, as they
have different attributes in their models. +
This enhancement also required to able to list out the data which either has error in processing or duplicate data in
the order list or return list to the user. This helps the user to identify the data and edit his CSV file data easily.
...
*** Highlights: Considerations are to be made for retrieving only the return order data from the list and store in hard
disk as a separate json file from the json file which store order data. Further considerations are also required to be
made for retrieving the return order data from the json file.
...
as by clearing the wrong list, the data in that list would be lost permanently.
