...
* `Override` the `getPrintableString` in classes `Name`, `Priority`, `Email`, and `Address` so that each produces a printable string representation of the object. e.g. `Name: John Smith`, `Priority: 12349862`
...
return getPrintableString(p.getPriority(), p.getEmail(), p.getAddress());
----
