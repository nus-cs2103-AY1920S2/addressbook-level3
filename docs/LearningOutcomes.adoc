...
* Assume you are planing to expand the functionality of the ExerciseList (but keep it as a CLI application).
What other user stories do you think ExerciseList should support? Add those user stories to the `DeveloperGuide.adoc`.
...
Assume that ExerciseList confirms the change with the user before carrying out the operation.
...
* Currently, ExerciseList data are saved to the file after every command.
...
Note how the `ExerciseList` class implements the `ReadOnlyExerciseList` interface so that clients who don't need write access to the `ExerciseList` can access the `ExerciseList` through the `ReadOnlyExerciseList` interface instead.

image::ReadOnlyExerciseListUsage.png[width=500]
...
* Do some enhancements to the ExerciseList GUI. e.g. add an application icon, change font size/style
...
* Is it possible to make the `ExerciseListParser` class more OCP-compliant in terms of extending it to handle more
command types?
...
=== Exercise: Enhance ExerciseList

* Enhance ExerciseList in some way. e.g. add a new command
