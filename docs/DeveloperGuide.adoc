...
By: `AY1920S2-CS2103T-W13-3`      Since: `Jun 2016`      Licence: `MIT`
...
=== Add Module / Deadline Feature

The `add` command adds the entity (module, deadline) with the fields provided by the user. `add m/module_code y/semester_index` adds a module and `add m/module_code y/semester_index t/task d/deadline` adds a deadline.
...
.. If it is the first time, a `Profile` object will be created using `NewCommand`.
. User enters the command. `AddCommandParser` will create a `AddCommand` object, which can contain `moduleCode`, `semesterIndex`, `grade`, `task`, `deadline`.
. `AddCommand#execute(Model model) is executed. `Profile#getHashmap()#values()` is called to check if the module already exists.
.. If module exists, user will be prompted to add a task and deadline.
.. If module does not exists, a `Personal` object will be created.
. After the module is added, a `CommandResult` containing the log of execution result will be returned.

==== Design Considerations

Aspect: How `add` executes

=== Show Module / Course / Focus Area / Semester Feature

The `show` feature allows the user to display information about a `Module`, `Course`, `Focus Area` or `Semester` with the command `show`, appended with the tags `m/module_code`, `c/course_name`, `f/focus_area` and `y/semester_index` respectively. These information cannot be seen from the main UI and have to be displayed through the `show` command.

==== Current Implementations

`show` command is parsed using `ShowCommandParser` to split the user input into relevant fields.

The following sequence diagram shows how the `show` operation works: `show c/Computer Science`

=== Edit Profile / Module Feature

The `edit` feature supports editing of `Profile` with the command `edit`, and editing of `Module` with the command `edit` appended with `m/module_code`.

==== Current Implementations

`edit` command is parsed using `EditCommandParser` to split the user input into relevant fields.

To edit `profile`, the command `edit` should be appended with one or more of the tags `n/name`, `c/course`, `cs/current_semester`.

To edit `module`, the command `edit` should be appended with `m/module`, followed by one or more of the tags `s/semester` and  `g/grade`.

`edit` command extends command class and uses inheritance to facilitate implementation. Hence it inherits the following methods:

- `Command#execute(Model model)`

The following sequence diagram shows how the `edit` command works: `edit m/CS2105 g/A`:
...
|`* * *` |Computing student |plan for specialisations |complete the requirements for my specialisation
