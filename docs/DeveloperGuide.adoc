...
If you wish to contact the Delino team, more details can be found <<AboutUs#, here>>.
...
[[insert]]
=== Insert Feature
In this section, the <<what-is-insert,functionality>> of the `insert` feature, the
expected <<insert-activity, execution path>>, the <<insert-class, structure>> of the *InsertCommand* class, the
<<insertparser-class, structure>> of the *InsertCommandParser* class and
the <<insert-sequence, interactions>> between objects with the *InsertCommand* object will be discussed.
...
not extensively utilised in *InsertCommand*; such as public static fields and getter/setter methods.
As shown in the diagram above, the *InsertParser* make use of methods from classes such as using the
...
[[list]]
=== List feature
In this section, the <<list-functionality,functionality>> of the `list` feature, the expected
<<list-path, execution path>>, the <<list-structure, structure>> of the *ListCommand* class
and the <<list-interactions, interactions>> between objects with the *ListCommand* object will be discussed.
...
The `delivered` function requires a valid <<command_flags, `FLAG`>> and a valid `INDEX`. +
i.e. `delivered` `INDEX` <<command_flags, `FLAG`>>

The <<command_flags, `FLAG`>> can either be '-o' or '-r', which indicates which list
(order list or return order list respectively) to mark the parcel from. The <<command_flags, `FLAG`>> is
...
on the <<command_flags, `FLAG`>>that is provided. For instance, if the '-o' <<command_flags, `FLAG`>> is provided,
...
<<command_flags, `FLAG`>> is present in the user's input. If the <<command_flags, `FLAG`>>
...
If a valid <<command_flags, `FLAG`>> is present, this will trigger the
...
that their input should follow. i.e. `delivered` <<command_flags, `FLAG`>>
 `INDEX` +

If both <<command_flags, `FLAG`>> and `INDEX` are valid, an *Order*
or *ReturnOrder* object will be created based on the <<command_flags, `FLAG`>>.
...
[[show]]
=== Show feature
In this section, the <<show-functionality, functionality>> of the `show` feature,
the expected <<show-path, execution path>>,
the <<show-structure, structure>> of the *ShowCommand* class and the <<show-interactions, interactions>>
between objects with the *ShowCommand* object will be discussed.
...
[none]
* 2c. Delino detects invalid <<command_prefix, `DELIVERY_TIMESTAMP`>>, i.e. order with
the given <<command_prefix, `DELIVERY_TIMESTAMP`>> is before the delivery time stamp of the order.
+
[none]
...
.. Insert command format: `insert` <<command_prefix, `TRANSACTION_ID`>> <<command_prefix, `CUSTOMER_NAME`>>
<<command_prefix, `ADDRESS`>> <<command_prefix, `PHONE_NUMBER`>> <<command_prefix, `EMAIL`>>
 <<command_prefix, `DELIVERY_TIMESTAMP`>> <<command_prefix, `WAREHOUSE_LOCATION`>> <<command_prefix, `CASH_ON_DELIVERY`>>
 <<command_prefix, `[COMMENTS_BY_CUSTOMER]`>> <<command_prefix, `[TYPE_OF_ITEM]`>>

.. Test case: `insert` `tid/9876543210` `n/John Doe` `a/Blk 572 Hougang st 51 #10-33 S530572` `p/98766789` `e/johndoe@example.com` `dts/2020-05-20 1300` `w/Yishun` `cod/$4` +
   Expected: Inserts an order with the above details to the list and displayed on the GUI
.. Test case: `insert` `tid/1023456789` `n/Amos Cheong` `a/Blk 572 Hougang st 51 #11-37 S530572` `p/90010019` `e/amoscheong@example.com` `dts/2020-05-10 1650` `w/Marsiling` `cod/$5` `c/Leave it at the riser` `type/glass` +
...
.. Test case: `return` `tid/9876543210` `n/John Doe` `a/Blk 572 Hougang st 51 #10-33 S530572` `p/98766789` `e/johndoe@example.com` `rts/2020-05-20 1300` `w/Yishun` +
...
.. Test case: `return` `tid/1023456789` `n/Amos Cheong` `a/Blk 572 Hougang st 51 #11-37 S530572` `p/90010019` `e/amoscheong@example.com` `rts/2020-05-10 1650` `w/Marsiling` `c/Leave it at the riser` `type/glass` +
...
.. Test case: Return order with invalid return time stamp +
...
. Mark a parcel's delivery status as delivered based on its `INDEX` and <<command_flags, `FLAG`>>
whenever the parcel has been delivered.

.. Delivered command format: `delivered` <<command_flags, `FLAG`>> `INDEX`
