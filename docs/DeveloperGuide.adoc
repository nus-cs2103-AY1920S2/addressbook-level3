...
* these 3 sets of data hava close interaction between each other through various commands input.
...
[NOTE]
As a more OOP model, we can store a `Offer` list in `Address Book`, which `Supplier` can reference. This would allow `Address Book` to only require one `Offer` object per unique `Offer`, instead of each `Supplier` needing their own `Offer` object. An example of how such a model may look like is given below. +
 +
