...
This feature allows users to record the exercises done by a client. The exercises are displayed in a table form, after the command `view-c INDEX` is called.
...
An important point to note about our implementation of `Exercise` is the method, `isSameExercise()`.

1. Executes when adding a new `Exercise` to client's `UniqueExerciseList`. This includes `add-e` and `edit-e`.
2. Checks if the new `Exercise` is considered a duplicate with an existing instance of `Exercise` in the client's `UniqueExerciseList`. 
...
The two `Exercise` only have different values in `ExerciseSets` but the same values in:
...
This implementation is chosen as we felt that the user should increment the value in `ExerciseSets` in the existing `Exercise`. We do consider that the user might want to record the two instances separately as it might be done at different periods of the day. However, we want to keep the exercises in the table compact and the user can easily view what exercises were done on that day.
...
The `FitBizParser` will create a `AddExerciseCommandParser`. By using `ParserUtil`, `AddExerciseCommandParser` will extract details like the reps of the exercise and pass the arguments to `AddExerciseCommand`.

For the `add-e` command, it is important which index the new Exercise is added. This is such that the exercises for the client will be displayed in descending chronological order in the table after a `view-c` command. `TableView` provides sorting for dates. However, having `TableView` to do the sorting would result in mismatch of indexes of the exercises in the `UniqueExerciseList` and in the `TableView`. This will result in problems when using `delete-e` command as it deletes the exercise at the specified index.

To address this problem, we implemented our add function, `addToSorted()`. Rather than calling the predefined `add()` from the `List` class, we will call `addToSorted()` to add our new instance of `Exercise`. Below shows a code snippet of `addToSorted()`.
