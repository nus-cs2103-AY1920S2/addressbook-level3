...
Note: This diagram assumes that a student with the matric number `A0181234G` exists
in the TA-Tracker.
...
`session done 1`. This session being called `done` has a recurring week of 1 week.
...
2. The `TaTrackerParser` sees that the command is a *sessions* type and passes the command
to the `SessionCommandParser`.

3. The `SessionCommandParser` sees that the command is a *done* type and passes the
...
checks whether the current session called by the user has a recurring period.
    ...
    b. If it does not has a recurring period, it will move on to *Step 6*.

6. The current session will be removed from `Model#UniqueSessionList`.

7. The updated session list will be displayed to the user.

.Activity Diagram for Done Session
...
{insert Filter Claims sequence diagram here}
...
When the user specifies a keyword, sessions that contain the keywords will be filtered and shown.
...
. Filtering students based on keywords.

.. Test Case: `student filter m/CS1101S` +
Expected: Shows all students that are under the first group of *module* `CS1101S`.
.. Test Case: `session filter m/CS1101S g/G06` +
Expected: Shows all students that are under *group* `G06` of *module* `CS1101S`.
