...
* An alternative implementation would be to a `UniqueModuleList` for all modules, a `list`
for all groups and a `list` for all students and then filter by module code or group
code according to how the user would want it to be.

* While this would be easier to
implement, it would also introduce cyclic dependencies as not only would students
need to keep a track of their groups, but groups would also keep a track of their students.

* Furthermore, an issue would be that there would be multiple groups with the same group
code in the list containing all groups as multiple modules can share the same group code.

* Similarly, there would be multiple instances of the same student in the list as the
same student could be a part of multiple groups.
====
...
8. The `AddGroupCommand` returns a `CommandResult` with a success message.
...
.. If it is not present, an exception is thrown explaining that the module doesn't
exist.
. If the module does exist, the module is retrieved from the TA-Tracker's model.
. The AddGroupCommand object checks whether a group with the same group code as the new group exists in
...
. If such a group doesn't exist, the group is added to the module.
. A command result with an appropriate success message is returned.
...
4. The `DeleteGroupCommandParser` creates a `Mmdule` with the given module code and
