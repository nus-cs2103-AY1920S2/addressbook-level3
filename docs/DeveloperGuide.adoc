...
For `session add` command,
these following steps are taken once the execute command of an `AddSessionCommand` object is called:

The `AddSessionCommand` object checks whether the module is present in the model of the TA-Tracker .

If it exists, the session is created.
...
The *Statistics Report Window* can be generated and displayed using the `report` command.
...
Hence, `TaTrackerParser` creates a `ShowStatisticCommandParser` object and passes the command argument `CS3247`
...
by creating a `Statistic` object that retreives data necessary for generating the report, from `ReadOnlyTaTracker`. These data is then processed further by `Statistic`. This includes computing the total number of sessions per session type and sorting the students by rating.
...
2. TA-Tracker retreives the list of completed sessions, and students.
3. TA-Tracker processes the data retreived.
...
[none]
* 1a. The user did not specify a specific module code.
[none]
** Use case resumes at step 2.
[none]
* 1b. The specified module code does not exist.
[none]
