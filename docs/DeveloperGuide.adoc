...
* The `Logic` component of the `Sort` commands differs from other components as the
`Module`, `Group` and `Student` components have their own parsers (such as the `ModuleCommandParser`,
`GroupCommandParser` and `StudentCommandParser` respectively) which direct the commands
towards their own individual parsers. The `Sort` commands, on the other hand, have
a single `SortCommandParser` which handles the parsing for all three kinds of commands.
====
