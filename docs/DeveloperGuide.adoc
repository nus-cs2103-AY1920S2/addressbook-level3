...
that are expiring over a month. It is facilitated by `CalendarPane`, `MonthView` and `DateCell`.

The `CalendarPane` is controller of the `Calendar`. Users can change the month
on display` to show the coupons that expire on a date by clicking on a highlighted
`DateCell`.
...
in the app so that the user can still keep track of it, and the savings it generated.
