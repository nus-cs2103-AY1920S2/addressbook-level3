...
. STASH informs the user that the entry is added.
+
Use case ends.
...
=== **Use Case: UC2 - Listing all the coupon entries**
**Actor**: `user`
...
[none]
* 1b. STASH detects that the data list is empty.
[none]
...
This use case describes how a user uses STASH to mark a coupon entry as used.

**MSS**

. User keys in command to mark a coupon as used.
...
. Matched coupons are displayed to the user.
+
Use case ends.
