...
|`* * *` |user |add items to the system |

|`* * *` |user |delete my items from the system |

|`* * *` |user | edit my item details in the system |keep the list updated

|`* * *` |user |view all items in my shop |keep track of my items

|`* *` |user |find items by keyword and attributes |search items more efficiently

|`* *` |user |receive notifications when the product quantity is running low, with the product name and balance left |stock up for the particular product

|`* *` |user |set the inventory quantity low limit |get notified when my stock is running low

|`* *` |user |view the top-selling products and worst-selling products at one glance (e.g. dashboard that displays name of products) |

|`* *` |user |view the statistics of each individual product, such as revenue made and sales volume |

|`* *` |user |see predicted sales for the future months based on past sales |know which products to stock up on

|`* *` |user |view a list of products sorted by the amount of sales in a selected period (e.g. last week/last month) |

|`* *` |user |view the revenue on a daily/monthly/yearly basis or in a customised period |

|`*` |user |generate a demographic report on customers |know who to target for marketing strategies

|`*` |lazy user |keep track of previous inputs |enter/edit previous commands easily

|`*` |lazy user |access products that are running low easily |restock and update the system much faster
...
[discrete]
=== UC02 - Add an item (product/ customer/ transaction)
...
4. The app shows the predictions.
...
[none]
* 3a. There are no products, hence no predictions can be made.
+
[none]
** 3a1. The app shows an error message.
+
...
[discrete]
=== Use case:  UC07 - Configure low inventory notification settings (products only)
...
[[customer]] Customer::
A person that buys the user’s shop
