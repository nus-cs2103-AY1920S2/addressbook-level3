...
* After setting the predicate on the model, the FilteredList will apply the Test method of the predicate and only display tasks with a score < 2. Scoring will be discussed in the next section.
* Model will also generate a searchComparator based on the score of applying the Predicate on a Task
** This comparator is then used to display more relevant search results to the user
* Any existing comparator set is removed in the `sortSearchByRelevance` call
...
// TODO update tag scoring

===== Name scoring
* We perform the filtering by taking chunks of the task name 
...
** The minimum of these scores is as the task's overall relevance score
** a smaller score means a more relevant task
* The calculation of a chunk is given by:
