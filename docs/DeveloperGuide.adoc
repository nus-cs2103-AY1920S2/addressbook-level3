...
* `CommandTextField`'s text is replaced by the suggested command
...
* `CommandTextField` is set to green
* `CommandTextField`'s text is unchanged
...
* We've enabled searching by phrase so that users can be more specific when looking for tasks

==== Scoring

image::PredicateSequenceDiagram.png[]

* We perform the match by taking chunks of the task name whereby the number of words in a chunk = number of words used in the search term
* We iterate through all chunks of the task name and calculate a score for that task
* The score is used to determine its relevance to the search term
** smaller score is more relevant
* The calculation is decided by:
** If edit distance < 2, score is set to 1
** If the search term matches the start of the chunk, score is set to 1
** If the chunk = search term, score is set to 0
* The minimum score of all chunks is set as the score of the Task
