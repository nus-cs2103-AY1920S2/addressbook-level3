...
Actor: User +
This use case describes how a user uses STASH to add
and save a new coupon entry. +
...
...  STASH requests the user to enter the details with the correct format.
...  User enters new data. +

Steps 1a.i- 1a.ii are repeated until the data entered are correct. Use case resumes from step 2.

1b) STASH detects that the specified coupon does not exist.

... STASH requests the user to enter an index that corresponds with an existing coupon.
...  User enters new index +
