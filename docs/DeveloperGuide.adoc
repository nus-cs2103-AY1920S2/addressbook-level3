...
=== AutoComplete
Auto complete is triggered when users press tab while on the command line. It serves to speed up the users usage of the application.
...
To handle the above case where an auto complete can be both a warning and a success, we update the color of the `CommandTextField` to reflect success and errors while only updating the color of the `ResultDisplay` if there are warnings.

* These color changes disappear after one second
* They are made in addition to changes to the text of both the `CommandTextField` and `ResultDisplay`
...
* If the model has existing comparators set via the SortCommand, they are applied for search results of equal relevancy
