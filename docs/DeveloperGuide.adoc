...
6. `LogicManager` calls `DeleteStudentCommand's` execute method. The `DeleteStudentCommand` object
...
The `session add`, `session edit` and `session delete` commands have been implemented in a similar manner.
...
The following _Sequence Diagram_ shows the sequence of commands that take place between the `Logic`
and `Model` components of the TA-Tracker when the user enters the command `student filter m/CS2103T g/G06`.
...
*Step 4:* The `FilterStudentCommandParser` creates a `FilterStudentCommand` object
with the relevant keywords.

*Step 5:* `LogicManager` calls `FilterStudentCommand` 's execute method.
The `FilterStudentCommand` object checks whether any of the keywords given by the user
matches the existing module and/or group.
If it does, the `FilterStudentCommand` returns a `CommandResult`
with a success message.
...
(For all use cases below, the *System* is the `TA-Tracker
...
1.  User requests to switch to a different `tab`.
2. TA-Tracker
layout changes to show the new tab.
...
2. TA-Tracker
opens a new window showing the list of commands.
+
Use case ends.
...
2. TA-Tracker
changes the pay rate.
3. TA-Tracker
shows an edited TSS claims page with the total earnings adjusted to reflect the new pay rate.
...
adds new student.
3. TA-Tracker
layout changes to show the student tab.
...
2. TA-Tracker
shows adds a new tutorial linked to the specified module.
3. TA-Tracker
layout changes to show the session tab.
...
2. TA-Tracker
shows a list of modules in the student tab.
3. User requests to delete a specific module in the list.
4. TA-Tracker
deletes the module and all of the sessions and groups in it.
+
Use case ends.
...
3. TA-Tracker
adds the new session into the session list.
4. TA-Tracker
switches to the `Session Tab` in order to display the new session.
+
Use case ends.
...
[none]
.. 1a1. TA-Tracker
creates a new session, and labels it as recurring.
+
Use case resumes at step 3.
...
1. User requests to go to the *Session View*.
2. TA-Tracker
switches to the `Session Tab`.
3. User requests to delete a specific session in the list.
4. TA-Tracker
deletes the session.
5. TA-Tracker
displays the new session list.
+
Use case ends.
...
edits the session.
5. TA-Tracker
replaces the session with the new version.
+
Use case ends.
...
deletes the session.
5. TA-Tracker
removes the session from the TA-Tracker
.
+
Use case ends.
...
5. TA-Tracker
removes the session from the *Session view*.
6. TA-Tracker
adds the session to the *Claims View*.
6. TA-Tracker
switches to the `Claims Tab`.
+
Use case ends.
...
2. TA-Tracker
registers the student in the session group.
3. TA-Tracker
switches to the `Student Tab`.
4. TA-Tracker
shows the student in the student list for the session group.
+
Use case ends.
...
2. TA-Tracker
removes the student from the session group.
3. TA-Tracker
switches to the `Student Tab`.
4. TA-Tracker
shows that the student is removed from the student list for the session group.
+
Use case ends.
...
1. User requests to go to the *Session View*.
2. TA-Tracker
switches to `Session Tab`.
3. User requests to filter sessions specific to date/module code/session type.
4. TA-Tracker
retrieves a list of sessions containing the keyword in any of their fields.
5. TA-Tracker
shows the list of sessions.
+
Use case ends.
...
1. User requests to go to the *Claims View*.
2. TA-Tracker
switches to `Claims Tab`.
3. User requests to filter sessions specific to module code.
4. TA-Tracker
retrieves a list of sessions containing the keyword.
5. TA-Tracker
shows the list of sessions.
+
Use case ends.
...
automatically switched to the `Student Tab`.
...
[NOTE]
====
The test cases after this assume that your TA-Tracker
has a module with module code
CS1101S.
====
...
Expected: Assuming a module with module code `CS3243` exists in the TA-Tracker
and
...
to the group `G06` of the module `CS1101S` with a default rating of 3. If you were on a
different tab, you are automatically switched to the `Student Tab`.
...
Expected: Changes the rating of the student with matric number `A123456X` to 4. If you were
on a different tab, you are automatically switched to the `Student Tab`.
...
tab, you are automatically switched to the `Student Tab`.
...
but the groups and students inside it will remain intact. If you were on a different tab,
you are automatically switched to the `Student Tab`.
...
of the group at index 1 of the module's group list. If you were on a different tab, you are
automatically switched to the `Student Tab`.
...
Expected: You can now view the groups of the module `CS1101S`. You will see the students
of the group `G06`. You will be switched to the `Student Tab`.
...
You will be switched to the `Student Tab`.
...
at index 1 of the module list. You will be
switched to the `Student Tab`.
...
of the module `m/CS1101S`. If you were on a different tab, you are
automatically switched to the `Student Tab`.
...
with the module `CS1101S` and be a `consultation` with Alice and Bob. If you were on a
different tab, you are automatically switched to the `Session Tab`.
...
period. The session marked as done will be added to the claims list. If you were on a
different tab, you are automatically switched to the `Claims Tab`.
...
Expected: Deletes the session at index `1` of the session list. If you were on a
different tab, you are automatically switched to the `Session Tab`.
...
Expected: Edits the session at index `1` of the session list to be of type `lab`. If you were on a
different tab, you are automatically switched to the `Session Tab`.

=== Filtering sessions

. Filtering sessions based on keywords. If you were on a
different tab, you are automatically switched to the `Session Tab`.
...
. Filtering claims by module code. If you were on a
different tab, you are automatically switched to the `Claims Tab`.
...
changed accordingly. You will be switched to
the `Claims Tab`.
...
groups, students and sessions related to this module. If you were on a
different tab, you are automatically switched to the `Student Tab`.
.. Test Case: `module delete m/CS1101S` +
