...
However, for the failure case, should any of the conditions fail, this means that either the specified objects does `not exist`, or
both entities are `already assigned` to each other or more importantly, that the model is in an `inconsistent state` where one entity is assigned to the other but not vice versa.
