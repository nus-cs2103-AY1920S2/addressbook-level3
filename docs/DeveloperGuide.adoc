...
.. Test case: `(ab)addnote 2 i/Likes red i/Lives in Jurong i/Cycles` +
   Expected: Second contact in the list has 2 new notes, "Likes red", "Lives in Jurong" and "Cycles", added. Details of the edited contact shown in the status message.
...
. A restaurant has yet to be visited

.. Test case: `(rt)visited 1` +
   Expected: Changes the visited status of the restaurant at index 1. Details of the edited restaurant shown in the status message.

. A restaurant has been visited

.. Test case: `(rt)visited 1` +
   Expected: The restaurant visited status will not change. Error details shown in the status message.
