...
.Scoring decision
The score is given by name score - tag score. Only tasks with scores < 2 will be displayed.

The final score is then used tp determine it's relevance in the search results by sorting the fitered tasklist in ascending score order.

image::PredicateSequenceDiagram.png[]

===== Name scoring
* The name score of a task is the minimum of the scores of each of a task's chunks
...
* A task is displayed if one of the three criteria are met and the name score is set to below 2:
...
* If none of the three criteria are met, the name score is set to 2
** The task is then not displayed unless it's tag score is positive
...
* We also chose to display task names who's start matches the search term for users to find tasks without typing the task's entire name
** Even though the edit distance could be significant
//end::branson[]
