...
===== Implementation
This section explains how the favourite and unfavourite commands are implemented. For brevity,

1. User enters the indexes of recipes that he wishes to favourite or unfavourite. The user input will be received by
`FavouriteCommandParser#parse(String args)` and `UnfavouriteCommandParser#parse(String args)` respectively.

2. The respective parsers will pass on the user input to `ParserUtil#parseMultipleIndex(String oneBasedIndex)` to
verify if the indexes keyed in are non-zero, unsigned integers. An error is thrown if any of the indexes do not meet
this requirement.

3. On top of verifying the validity of the indexes, `ParserUtil#parseMultipleIndex(String oneBasedIndex)` will remove
any duplicate indexes and sort them. It finally returns a sorted array of one-based indexes.

4. A new `FavouriteCommand` or `UnfavouriteCommand` object will be created with the array of indexes and returned to
the `LogicManager`.

5. The `FavouriteCommand#execute(model)` method is executed and `Model#updateFilteredRecipeList(p)` is called. This tests
every recipe in the database against the predicate, p, and updates the filtered recipe list with recipes that meet the
user specified criteria.

6. Once complete, this filtered recipe list is displayed to the user.
...
The following commands were implemented to overcome the limitations of the `edit` command. These recipe customisation
commands allow the user to make targeted changes to the ingredient or step fields instead of having to rewrite the
entire field using `edit`.

==== Add ingredient command
===== Implementation
This section explains how the add ingredient command is implemented.

1. User specified keywords are directed to the `AddIngredientCommandParser` where `ArgumentTokenizer` and
...
1. User specified keywords are directed to `FilterCommandParser#parse(String args)` where `ArgumentTokenizer` and
`ArgumentMultimap` are used to parse the user input. An error will be thrown if no keywords are specified at all.
