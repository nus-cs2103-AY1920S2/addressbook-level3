...
Priorities: epic++ (must have) - * * * *, rare++ (nice to have) - * * * , comon (unlikely to have) - * *, rabak (will negatively affect the application) - *

.User stories and their priorities
[width="59%",cols="22%,<23%,<25%,<30%",options="header",]
|====================================================================================================================================================================
|Priority |As a ... |I want to ... |so that I can ...
|* * * * |forgetful student |keep track of all the _promo codes_/coupons that I collected |use them at their respective shops
|* * * * |_SoC_ student |quickly input the coupons that I collected from welfare packs |have a digital record of all the vouchers I have
...
|* * * * |user |track how much I have saved from using these _promo codes_/coupons |know how much I save
|* * * * |thrifty student with student loan |discover and apply discount codes/coupons |maximise my savings
|* * * * |highly competent _SoC_ student |showcase how easy it is to use command-line |execute simple tasks like add, sorting and finding a coupon
|* * * * |organized student |have a easy visualisation representation of all my coupons |
...
|* * * |user |track how much I have spent from using these _promo codes_/coupons |
|* * * |financial-aid _SoC_ student |quickly store the _promo code_ shared by my peers and use them later for critical necessity like KBBQ and escape room |
|* * * |exchange student attached to _SoC_ |keep track of the good deals in Singapore and visit them |
|* * * |time-conscious student |using command line to access my coupons |save time
|* * * |lazy student |easy input of coupon details |
|* * * |influencer |import my friend's coupons |
|* * * |forgetful user |search prediction |
|* * * |digital nomad |share coupons easily with friends while I'm on the go |
...
|* * |block head |share coupons to all my hall members |
...
|* |mother of 5 _SoC_ students |always looking out for the best and hottest deals in town |finance my childrens’ education
|* |computing student |data saved by the app to be easily parsable |create alternative clients
|* |infosec student |coupon data to be encrypted |hackers cannot hack my coupons
|====================================================================================================================================================================
...
image::usecase.png[Use Case Diagram,500,200]

[discrete]
=== **_Use Case: UC1 - Add  a new coupon entry_**

Actor: User +
This use case describes how a user uses STASH to add and save a new coupon entry. +

MSS:

[start=1]
. User keys in command to add a coupon entry.
. STASH evaluates input details and adds the coupon entry.
. STASH inform user that entry is added

=== **_Use Case: UC2 - Listing all the coupon entries_**

This use case describes how a user uses STASH to list all the coupon entries. +

MSS:

[start=1]
. User keys in command to list all the coupons.
. STASH evaluates input details and prints out all coupons. +

Extensions:

[start=2]
. STASH detects that the data list is empty. +
.. STASH will inform the user that the list is empty. +
.. Use case ends
...
. User keys in command to edit an existing coupon.
. A confirmation pop will be displayed to the user to confirm its decision.
. Once confirmed, the coupon’s details are changed to follow the users input.
. STASH informs the user that the coupon’s details have been changed.
+
Use case ends.
...
[none]
** 1a1. STASH requests the user to enter the details with the correct format.
...
. User keys in command to set reminders for an existing coupon.
. On the day of the input date, a pop up will appear and remind the user about the coupon and its details.
+
Use case ends.
...
. Matched coupons are displayed for the user.
+
Use case ends.
...
. User lists all the coupons (UC2) or Find coupon(s) by keywords(s) (UC4) or lists coupons before the expiry date (UC7).
. User keys in command to delete an existing coupon.
. A confirmation pop will be displayed to the user to confirm its decision.
. STASH informs the user that the coupon has been removed.
+
Use case ends.
...
. A confirmation pop will be displayed to the user to confirm its decision.
. Once confirmed, STASH will undo the previous command and inform the user that undo operation is successful.
+
Use case ends.
...
. Coupon Stash can hold at least 500 coupons comfortably, and still operate without noticeable lag when entering commands or interacting with the UI.
...
. Data files should remain unchanged when moving from OS to OS.
...
Terms that are italicised inline code (e.g `_operating system_`) can be found with definitions here.
...
. **UI (User Interface)** - a catch-all term referring to how a computer system and a person interacts, usually referring to specific elements displayed on the computer screen that the user may interact with such as buttons or text boxes, as well as areas where the computer application displays certain outputs to the user.
=======
