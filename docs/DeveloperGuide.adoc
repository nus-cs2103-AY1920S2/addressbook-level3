...
Similarly, since this feature relies heavily on the UI class `CommandBox`, we have decided to let `CommandBox` interact with `AutoComplete` directly. The sequence diagram for when the user presses the kbd:[Tab] key is given as such:
