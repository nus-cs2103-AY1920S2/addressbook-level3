...
.Structure of the StudentTab Component
...
* exposes 5 unmodifiable `ObservableLists<>`:
...
1.  User requests to goto a different tab.
...
3. TA-Tracker layout changes to show the student taba.
...
. TA-Tracker shows a list of students at the student tab.
...
2. TA-Tracker shows a list of modules.
...
. User filters the list in order to get the group's index.
...
3. TA-Tracker adds the new session into the corresponding module group.
...
[none]
. 2b. The session type does not exist.
[none]
...
1. User requests to view the Session Tab.
...
* 3a. The given session index is invalid.
** 3a1. TA-Tracker shows an error message.
+
Use case resumes at step 2.
...
2. TA-Tracker switches to the Schedule Tab.
...
6. TA-Tracker switched to the Claims Tab.
+
Use case ends.
...
=== Use case: Filter claims for a specific module

*MSS*

1. User requests to filter claims for a specific module.
2. TA-Tracker hides unrelated claims from the Claims view.
3. TA-Tracker switched to the Claims tab.
+
Use case ends.
