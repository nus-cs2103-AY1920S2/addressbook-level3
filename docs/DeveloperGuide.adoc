...
The following syntax of the `InsertCommand` is : +

`insert` <<command_prefix, `TRANSACTION_ID`>> <<command_prefix, `NAME`>> <<command_prefix, `ADDRESS`>> <<command_prefix, `PHONE_NUMBER`>>
<<command_prefix, `EMAIL`>> <<command_prefix, `DELIVERY_TIMESTAMP`>> <<command_prefix, `WAREHOUSE_LOCATION`>>
<<command_prefix, `CASH_ON_DELIVERY`>> <<command_prefix, `[COMMENTS]`>> <<command_prefix, `[ITEM_TYPE]`>>
...
The above sequence diagram illustrate how `Clear` Command being processed when the user input `clear -f` to force clear
both order book and return order book.
...
some mehods and variables omitted from this diagram as it is irrelevant to the `ShowCommand` class.
...
provided is either `all` or `today` or simply just a `DATE` the value of `DATE` will be validated t check if it is a valid date.
