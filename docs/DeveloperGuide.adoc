...
Our module search function consists of 2 main implementation 
  ...
  The implementation allows user to retrieve details about a module from NUSmods if the user have not query about the module before. The search function will create a API call and retrieve the object as a JSON object, which will be decoded and parsed into different relevant attribute of the Module Class. 
  
  The moduleSearch function would first search its storage in data/addressbook.json whether the module information is queried before. If queried, the information would be retrieved from the storage instead of querying from the internet. This allows user to plan for module if they have queried the module before without the need for Internet access.
   

=== [Proposed] Pathing

Given below is the consideration and proposed implementation for pathing in the future versions.
...
AbsolutePath will take the path given from `/`.

RelativePath will take reference from the path that the current system has opened.

The current implementation for the notes module allows user to access from `usr/Desktop/{}` to open, create and list the directory.

In future implementation we are considering to implement the option to choose from AbsolutePath and RelativePath.
...
Will require much more input from the user
