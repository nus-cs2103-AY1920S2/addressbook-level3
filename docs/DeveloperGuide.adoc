...
==== Saved Data
The following _Class Diagram_ shows a breakdown of the TA-Tracker data managed by the `Storage` component.
...
* This diagram assumes that there is a student with the matric number `A0181234G` in the TA-Tracker
...
4. The `DeleteStudentCommandParser` creates a `Student` object with the given matric number,
`A0181234G`. It also extracts the *module code* and *group code* from the arguments.

5. The `DeleteStudentCommandParser` then creates a `DeleteStudentCommand` object
using the created `Student` object, *module code* and *group code*.
...
A user input can be divided into the following parts:
...
`prefix`
|
The part of an `argument` up to and including the `/` foward slash delimiter.
|
image::SyntaxHighlightingClassDiagram.png[]
...
the `ResultDisplay` will indicate that a correct command has been entered.
|
...
|
.A trailing whitespace (coloured in blue for this diagram)
...
[[Syntax-Aspect1]]
===== Aspect #1: Showing the command usage after every two white spaces
...
[[Syntax-Aspect2]]
===== Aspect #2: Always showing the syntax highlighting

Similar to <<Syntax-Aspect1, Aspect #1>>, the syntax highlighting could be toggled
