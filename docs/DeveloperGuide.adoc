...
It also exposes four unmodifiable lists that can be 'observed' by the `UI`: +
...
loop through for for each recipe. The checks ensure that all the recipes inputted are valid, else the  `CookedCommand`
...
With reference to the structure of the CookedRecords Model (<<Structure of the Record Model>>),
