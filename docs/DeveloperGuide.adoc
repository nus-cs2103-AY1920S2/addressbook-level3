...
Recording the clients' exercises is one of the main features of our application. This section provides its key implementation details.

==== Implementation

===== Exercise class

We implemented the `Exercise` class to contain the details of the exercise.  
...
All instances of `Exercise` of a client will be contained in the client's `UniqueExerciseList`.

Below shows a UML class diagram which provides further understanding of `Exercise`.

image::ExerciseClassDiagram.png[]

An important point to note about our implementation of `Exercise` is the method `isSameExercise`. This method is used when adding a new `Exercise`. It checks if the new `Exercise` is considered a duplicate with an existing instance of `Exercise` in the client's `UniqueExerciseList`. //aout equals. Two `Exercise` are considered to be the same if they have the same: +
