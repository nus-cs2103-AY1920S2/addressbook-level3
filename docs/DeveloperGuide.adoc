...
==== Implementation
The tasklist has no default sorting set on it. It will only be sorted when the sort command is used like so `sort {field1}, {field2}, ...`. To implement this, we've augmented the UniqueTaskList and TaskList with a setComparator. This allows us to set several comparators on the taskList to perform multi level sorting while maintaing the ObservableList which JavaFX can then use to update the UI.
