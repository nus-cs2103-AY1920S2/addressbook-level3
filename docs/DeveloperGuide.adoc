...
. `ReportCommandParser` then parse an optional user input to determine the period of report to display. +
This Process returns a `ReportCommand` object to the `LogicManager`.
. The `LogicManager` then access `ReportCommand#execute()`.
. The `ReportCommand#execute()` retrieves the transaction list from storage and display a list adapted to a report format within the given period, or all transactions if the period is not given.
...
. The `SelectItemCommand#execute()` retrieve the selected `Food` and creates a `PurchasedFood` object with additional variables.
...
image::TransactionsActivityDiagram.png[]
...
. `TransactionsCommandParser` then parse an optional user input to determine the period of transaction report to display. +
