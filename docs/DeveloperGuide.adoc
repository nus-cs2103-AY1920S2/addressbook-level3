...
As explained in 2.3, a `pre-processing step` must first be done before executing an undo-able assign/unassign command
to ensure that all entity links are in correct state before command execution.

.Relationship between Entities
image::IVAN-ProprocessingAssignActivityDiagram.png[]
