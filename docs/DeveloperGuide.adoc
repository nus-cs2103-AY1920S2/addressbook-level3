...
. Add a recipe with one mandatory field missing.
...
. Add a recipe with a missing ingredient quantity
...
.. Test Case: `modify 1 cal/2000` +
   Expected: The first recipe's original calories should be updated to 2000 kCal.
...
.. Prerequisite: *CookBuddy* contains a recipe with _bread_ in its ingredients. +
   ...
   Expected: Recipes with the word _bread_ in their ingredients are listed.

// finding a recipe by instruction
...
=== Marking / un-marking a recipe as a favourite
...
.. Test case: `delete y` (where y is not an integer) +
