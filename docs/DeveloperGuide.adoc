...
the command `mdelete m/CS2103T`.

.Component interactions for `mdelete m/CS2103T` command
...
The UI consists of a `MainWindow` that is made up of parts e.g.`HelpWindow`, `TabPanel`,
...
.Interactions Inside the Logic Component for the `mdelete m/CS2103T` Command
...
The undo/redo mechanism is facilitated by `VersionedNasaBook`.
It extends `NasaBook` with an undo/redo history, stored internally as an `NasaBookStateList` and
`currentStatePointer`.
Additionally, it implements the following operations:

* `VersionedNasaBook#commit()` -- Saves the current NASA book state in its history.
* `VersionedNasaBook#undo()` -- Restores the previous NASA book state from its history.
* `VersionedNasaBook#redo()` -- Restores a previously undone NASA book state from its history.

These operations are exposed in the `Model` interface as `Model#commitNasaBook()`, `Model#undoNasaBook()` and
`Model#redoNasaBook()` respectively.

Given below is an example usage scenario and how the undo/redo mechanism behaves at each step.

Step 1. The user launches the application for the first time. The `VersionedNasaBook` will be initialized with the
...
`Model#commitNasaBook()`, causing the modified state of the NASA book after the `delete 5` command executes to be
saved
...
Step 3. The user executes `add n/David ...` to add a new module. The `add` command also calls `Model#commitNasaBook()
`, causing another modified NASA book state to be saved into the `NASABookStateList`.

image::UndoRedoState2.png[]

[NOTE]
If a command fails its execution, it will not call `Model#commitNasaBook()`, so the NASA book state will not be saved
...
`undo` command. The `undo` command will call `Model#undoNasaBook()`, which will shift the `currentStatePointer` once
...
book
states to restore. The `undo` command uses `Model#canUndoNasaBook()` to check if this is the case. If so, it will return
...
The `redo` command does the opposite -- it calls `Model#redoNasaBook()`, which shifts the `currentStatePointer` once
...
there
are no undone NASA book states to restore. The `redo` command uses `Model#canRedoNasaBook()` to check if this is the
...
will usually not call `Model#commitNasaBook()`, `Model#undoNasaBook()` or `Model#redoNasaBook()`. Thus, the
`NASABookStateList` remains unchanged.

image::UndoRedoState4.png[]

Step 6. The user executes `clear`, which calls `Model#commitNasaBook()`. Since the `currentStatePointer` is not
...
`HistoryManager` and `VersionedNasaBook`.
...
The figure below shows the sequence diagram on what happens with the execution of the aforementioned command.
image::StatisticsLogicSequenceDiagram.png[]
