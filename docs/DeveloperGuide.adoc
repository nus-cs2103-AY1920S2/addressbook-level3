...
Similar to <<Command History --- Aaron Choo, Command History>> , this feature also relies heavily on the UI class `CommandBox`, and thus we have decided to let `CommandBox` interact with `AutoComplete` directly. In the following sequence diagram, we follow the execution for when the user tries to autocomplete his partially entered command `sch` (which in the current state of the application is an unambigous command, and will result in the full completion of the `schedule` command as well as the prefixes):

// TODO: do sequence diagram for auto complete

This feature currently only supports autocompletion of _commands_ and _prefixes_, and not other fields/parameters like names and addresses that have been used by the user before. Implicitly, since all commands defined in FitBiz do not have empty spaces in them, this allows us to easily determine when to enable <<command-history-behaviour-4, behaviour 4>> as defined above: by simply checking for the presence of white spaces from the trimmed user input. The following activity diagram is given to show the decisions made at each step of the execution:
