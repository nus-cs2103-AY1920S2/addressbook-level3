...
=== Automation
Every time a user stores an activity, it date of creation will be recorded. Based on user input to repeat the task either
weekly, twice weekly or monthly, our application will check whether today's date has passed `date of creation`. Once it passed,
the newly set date will be added from `date of creation` + `user input`.
...
, `activity name` to identify the activity and set the activity to `1-Weekly-, 2-Twice Weekly, 3-Monthly`.
...
. After that, it will find for the activity inside the module.
. Once found, the attribute Schedule in activity will be set to `1`, `2` or `3` and update changes to the expiring date.
...
Refer to the user guide for the specific commands. Sorting of activities is facilitated by ModelManager#sortActivity.
...
. The user enters the command `sort c/priority`
...
it The design considerations for the classes are shown below:
