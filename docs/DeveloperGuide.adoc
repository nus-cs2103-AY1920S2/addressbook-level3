...
==== Implementation
The tasklist has no default sorting set on it. It will only be sorted in the order defined through the sort command like so `sort {field1}, {field2}, ...`. To implement this, we've created a new class `SortedUniqueTaskList` which extends `UniqueTaskList`. This extended class accepts Comparators and sorts the internal task list as according to the comparator while maintaining the immutability of the items in the tasklist.
