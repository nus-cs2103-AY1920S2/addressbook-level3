...
The sort command takes in a list of fields and generates FieldComparators as seen in the diagram and then uses Comparator.thenComparing to aggregate the comparators. The first field provided will then be of the highest sort order. The Model will then set the same `Comparator` on the `TaskList` which internally sets it on a `SortedList` from JavaFx. This SortedList will then help us maintain a sorted order even when new or edited tasks are added.

.Class diagram of SortedList
image::SortedListClassDiagram.png[]
We had to use a new SortedList within TaskList because the FilteredList does not allow for sorting. As such we have to let the FilteredList reference the SortedList and the SortedList refernce the UniqueTaskList so that any changes to UniqueTaskList will go through both sorting and filtering.

.Sequence of initializing the Sorted List
image::SortInitliazeDiagram.png[]

Due to the requirements mentioned, this is how we had to create our FilteredList. We set FilteredList to reference the SortedList and then the SortedList to reference the UniqueTaskList.

==== Updating UI
image::SortUiSequenceDiagram.png[]

Meanwhile to update the UI on the newest sorting order, the SortCommand will return a `SortCommandResult`. This `SortCommandResult` contains an additional field which would be the highest sorting order of the TaskList. The `SortCommandResult` is then passed back to the `MainWindow` and to the `TaskListPanel`. The `TaskListPanel` then gets the sort order from `SortCommandResult` and updates the UI.
