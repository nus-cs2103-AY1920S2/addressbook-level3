...
* Alternative 2: Define new keywords that the user can use. These keywords could be "more english-like" as opposed to
using shortened tags as prefixes.
...
==== Implementation
This sections explains how the undo and redo commands are implemented.
...
** Cons: May use up a lot of memory space if the user performs many operations that commit the recipe book.
This effect is amplified if the database is huge (ie. contains many recipes) as a copy of the entire
recipe book is made after every commit.
