...
[IMPORTANT]
=====
Auto completion of a word happens when:
...
. attempts to complete the command word
.. if command word is unrecognized, throws a `CompletorException`
.. else it performs specific checks on the input based on the command word and auto completes as necessary
...
.. Auto complete fields based on the callout <<completion criteria, here>>
.. If cannot complete, then it is removed
