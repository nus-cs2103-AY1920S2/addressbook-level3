...
.. Delete command syntax: `delete` `*INDEX*`
...
.. Test case: `delete *ANY INDEX NOT FOUND IN LIST*` +
...
.. Edit Command Syntax: `edit` `*TRANSACTION ID*` `*FIELD*` `*NEW VALUE*`
...
.. Test case: `list *ANY WORD OTHER THAN UNDONE AND DONE*` +
...
. Mark order as done whenever an order transaction is completed. The correct syntax should be as follows: `done` `*INDEX*`
...
.. Clear command syntax: `clear` `[FLAG]`
