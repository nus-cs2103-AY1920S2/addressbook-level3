...
==== Implementation
This sections explains how the filter command is implemented.
...
The result of this operation is: All recipes that 1) are marked as favourites, 2) take 20 minutes or less to prepare,
and 3) contains pasta as an ingredient will be displayed.

==== Design Considerations

===== Aspect: Consistency of user input syntax
...
also prevents causing confusion to the user.
** Cons: The user has to be familiar with the prefixes and other special keywords in order to use this feature to its
fullest potential.

* Alternative 2: Define new keywords that the user can use. These keywords could be "more english-like" in nature as
opposed to using shortened tags as prefixes.
** Pros: It might be easier to remember these keywords since they are more english-like. The user then wouldn't have to
