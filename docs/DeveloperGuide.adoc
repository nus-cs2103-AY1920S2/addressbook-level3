...
All instances of `Exercise` of a client will be contained in the client's `UniqueExerciseList`. Below shows a UML class diagram which shows `Exercise` class interactions.

image::ExerciseClassDiagram.png[]

An important point to note about our implementation of `Exercise` is the method `isSameExercise`. This method is executes when adding a new `Exercise`. It checks if the new `Exercise` is considered a duplicate with an existing instance of `Exercise` in the client's `UniqueExerciseList`. Two `Exercise` are considered to be the same if they have the same: +
...
This implementation is chosen as we felt that the user should increment the `Set` value in the existing `Exercise`. We do consider that the user might want to record the two instances separately as it might be done at different periods of the day. However, we want to keep the exercises in the table compact and the user can easily view what exercises were done on that day.
...
image::AddExerciseCommandSequenceDiagramPart.png[]
...
We want to keep the implementation reading and storing of data simple. The first approach is  the most simple. When reading the data, it removes the need to associate the exercises to the client. A client might potentially have a large amount of exercises, resulting in the reading process to be extremely slow and a bad user experience.
