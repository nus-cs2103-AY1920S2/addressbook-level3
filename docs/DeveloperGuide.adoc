...
// TODO add diagram of how TaskListPanel is referencing FilteredList and FilteredList references TaskList which contains SortedList that is aware of the UniqueTaskList
...
==== Updating UI
image::SortUpdateUi.png[] // TODO add in
...
Auto complete is triggered when users press tab while on the command line. `CommandCompletor` first attempts to complete the command word, if it is unrecognized it throws a `CompletorException`. Otherwise it will perform specific checks on the input based on the command word. Here are the different checks:
...
. If input is a delete/done command then CommandCompletor will attempt to remove any invalid indices by checking against the length of the displayed task list and checking if the index is a positive integer with `StringUtil`
. If input is a sort command then CommandCompletor will try to complete fields by calculating the edit distance between the input and accepted sort fields. It will only correct fields with an edit distance <= 1

Auto Complete output:

. If words are completed, prefixes are added or nothing has changed, auto complete is labelled as successful and the UI updates to show the user a green textfield.
. As long as any input is deleted (invalid index/sort field), auto complete will raise a warning via feedback to user by turning the feedback to orange.
. Auto complete is labelled as an error -> no recognizable command word found.
. Auto complete is labelled as both successful and warning.
