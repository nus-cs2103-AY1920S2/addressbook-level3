...
| *Adhering to OOP standard* | Logically makes more sense, as each client has to have a list of exercises to themselves, and thus `PersonalBest` of each of these exercises |  Actually increases coupling as the logic was flawed 
| *Ease of Implementation* | Might have significant conflicts as the `Client` model is changed to include one more attribute | Keep `Exercise` data and methods separate from `Client` data and methods
