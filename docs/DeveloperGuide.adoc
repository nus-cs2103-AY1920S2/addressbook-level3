...
[NOTE]
As a more OOP model, we can store a `Tag` list in `FitHelper`, which `Person` can reference. This would allow `FitHelper` to only require one `Tag` object per unique `Tag`, instead of each `Person` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
...
* Each `Entry` consists of a unique combination of `Name`, `Calorie`, `Location`, `Duration`, `Type` and `Time`
