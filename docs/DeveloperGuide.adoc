...
An important point to note about our implementation of `Exercise` is the method `isSameExercise()`. This method executes when adding a new `Exercise`. It is used to check if the new `Exercise` is considered a duplicate with an existing instance of `Exercise` in the client's `UniqueExerciseList`. Below shows an object diagram of two `Exercises` that will return true for `isSameExercise()`.

image::IsSameExerciseObjectDiagram.png[]

The two `Exercise` have different values in `ExerciseSets` but the same values in:
