...
Indeed, `JsonAdaptedClient` having a list of `JsonAdaptedExercise` would violate separation of concerns. `JsonAdaptedClient` is now in-charge of the client's information and the exercises he did. However, we felt that the benefits outweighted the costs.
