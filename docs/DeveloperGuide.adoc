...
before and is now committing a new book, he will not be able to call redo those undos anymore). This is the behavior
that most modern desktop applications like Microsoft Word adopt.

3. `Model#commitBook` also saves the `CommandType` and `Tab` of the command in 2 separate stacks in
`MultipleBookStateManager`. It will also save the current state of the affected book(s) (before the command executes)
in an ArrayList of that book type.
...
The following diagrams show what happens when the user calls an undo operation:
