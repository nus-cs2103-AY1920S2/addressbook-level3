...
CookBuddy helps users familiar with the command-line and are fast typists, to easily manage their recipes. Features include adding, editing, and deleting recipes, as well as adding recipe images, tags, and setting various recipe attributes like serving size, cooking time, and personal management like ratings and favourites.
...
This section describes some noteworthy details on how certain features are implemented, as well as alternatives. Some possible future components are briefly covered, and these may be released in `v2.0`.
...
image::PhotoActivityDiagram.svg[align="center"]
