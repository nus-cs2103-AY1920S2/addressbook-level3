...
. NasaBookParser will call RepeatActivityCommandParser to check if all the following prefixes `m/`, and `r/` are present.
.. If not present, an error message `"invalid command format!"` will be prompted.
.. Else, a success message `"Successfully added a repeat! for CS3233 0 1"` will be given, and RepeatCommand will be executed.
