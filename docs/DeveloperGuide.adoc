...
* Update `responseText` of the `Model` (by the `SuggestionHandler`): for instance, the input `open /` will set the `responseText` in the `Model` as
"Open a note".
...
Given below is the Sequence Diagram for interactions within the `Logic` and `Suggestion` component for the `execute("open /a")` API call.

.Interactions Inside the Logic and Suggestion Component for the input `open /a`
