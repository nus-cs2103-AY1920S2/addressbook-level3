...
1. The `AddCommand#execute(Model model)` method is executed and a new `Food` would be created.
...
1. The `DeleteCommand#execute(Model model)` method is executed and it validates that the specified `INDEX` to delete
...
1. The `ListCommand#execute(Model model)` method is executed and it validates that the flag used to decide what
Foods to list. If the flag is valid, the items to be listed will be retrieved from FoodBookStorage according
to the input flag.

2. The method `model#updateFilteredFoodList() will then be called to retrieve the List of Foods from Storage.
...
1. The `EditCommand#execute(Model model)` method is executed and it validates that the specified `INDEX` to edit
...
1. The `BmiCommand#execute(Model model)` method is executed and it will return the output of the calculated BMI
...
Step 4. `BmiCommand` would then refer to the internal state of the model under Self, and retrieve the values stored in
...
* Alternative 1 (current choice): No storage of BMI value, simply prints when user requests.
...
1. The `HeightCommand#execute(Model model)` method is executed and it validates that the specified `HEIGHT` to store
...
1. The `WeightCommand#execute(Model model)` method is executed and it validates that the specified `WEIGHT` to store
...
1. The `ModeCommand#execute(Model model)` method is executed and it validates that the specified `MODE` to store
...
1. The `AddItemCommand#execute(Model model)` method is executed and it checks if the specified `ITEM` and `PERSONS` to
...
1. The `ListReceiptCommand#execute(Model model)` method is executed.
...
1. The `PaidCommand#execute(Model model)` method is executed.
...
1. The `BackCommand#execute(Model model)` method is executed.

2. The `CommandResult` with the `isBack` will set to true and returns to `EYLAH`.

3. The `EYLAH#runCommandLoopUntilExitCommand()` will check if `isBack` is set to true.
...
[[self]] Self:
A static class used in Diet Tracker to represent the user in the model.
