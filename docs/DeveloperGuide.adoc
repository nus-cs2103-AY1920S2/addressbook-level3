...
To address this problem, we implemented our add function, `addToSorted()`. Rather than calling the predefined `add()` from the `List` class, we will call `addToSorted()` to add our new instance of `Exercise`. 
...
The exercise list, `UniqueExerciseList`, goes through a loop and iterates through the exercises in the list. `UniqueExerciseList` then calls `getExerciseDate()` and `getExerciseName` to get the `LocalDate` and exercise name in `String` type for comparision. The sequence diagram below illustrates the conditional checks during the comparision.

image::AddExerciseCommandSequenceDiagramPart3.png[]

The conditional checks are such that `UniqueExerciseList` maintain sorted by descending chronological order, followed by alphabetical order for `Exercise` with the same dates.
...
It is possible to use `Arrays.sort()` to sort the exercises in the `UniqueExerciseList` after every addition. However, the worse case time complexity of `Arrays.sort()` is `O(nlogn)`. Whereas, our implemented function , in the worse case, runs at `O(n)`. `UniqueExerciseList` has to check if it contains the same `Exercise` and that already takes `O(n)`. It is also very possible for the clients to have many exercises so we have to keep the time complexity low.
