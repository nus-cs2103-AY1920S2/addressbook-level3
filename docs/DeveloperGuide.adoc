...
as `Done`. Similarly, if an `Assignment` has been mistakenly marked as done or
...
Finance type `CourseTeacher` or `ct` add command will access `StaffAddressBook` and `CourseAddressBook` to ensure `Staff` and
`Course` exist and make sure this staff is teaching this course. The `Amount` is set to the amount salary of the `Staff`.
