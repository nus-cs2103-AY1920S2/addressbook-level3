...
.. Other incorrect delete commands to try: `del m/` (key in module code), `del m/cs2030` (where cs is not in capital letters) _{give more}_ +
...
. `edit-d` is to edit a deadline activity while `edit-e` is to edit an event activity.
