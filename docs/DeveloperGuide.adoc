...
the total earnings of the user. Each time DoneSessionCommand is called,
TotalHours is incremented according to the duration of the completed session.
The duration of the completed session is calculated by subtracting the start
time of the session from the end time and then rounding up the value to a whole.
