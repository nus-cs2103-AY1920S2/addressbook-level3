...
The `address` field in `Recipe` is actually an instance of the `seedu.address.model.recipe.Address` class.
...
. Safe delete the field `address` in `EditRecipeDescriptor`
...
For example, removing the `address` field from the `Recipe` class will require you to modify its constructor.
