...
In scenarios like this, you can consider refactoring the existing `Recipe` model to suit your use case.

In this tutorial, we'll do exactly just that and remove the `recipe` field from `Recipe`.
...
The `recipe` field in `Recipe` is actually an instance of the `seedu.address.model.recipe.Address` class.
...
Other usages like in `EditRecipeDescriptor` may require more careful inspection.

Let's try removing references to `Address` in `EditRecipeDescriptor`.

. Safe delete the field `recipe` in `EditRecipeDescriptor`
...
. Remove the usages of `recipe` and select `Do refactor` when you are done.
+
TIP: Removing usages may result in errors.
Exercise discretion and fix them.
For example, removing the `recipe` field from the `Recipe` class will require you to modify its constructor.
...
You can find them by searching for instances of the word `recipe` in your code (`Edit` > `Find` > `Find in path`).
...
For example, there is a `$address` in each `RecipeCard` that has not been removed nor identified.
...
While keeping the `recipe` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
```
You can go through each individual `json` file and manually remove the `recipe` field.
