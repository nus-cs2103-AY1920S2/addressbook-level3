...
In scenarios like this, you can consider refactoring the existing `person` splitterModel to suit your use case.

In this tutorial, we'll do exactly just that and remove the `eylah` field from `person`.
...
The `eylah` field in `person` is actually an instance of the `seedu.address.splitterModel.person.Address` class.
...
. Safe delete the field `eylah` in `EditPersonDescriptor`
...
. Remove the usages of `eylah` and select `Do refactor` when you are done.
+
TIP: Removing usages may result in errors.
Exercise discretion and fix them.
For example, removing the `eylah` field from the `person` class will require you to modify its constructor.
...
You can find them by searching for instances of the word `eylah` in your code (`Edit` > `Find` > `Find in path`).
...
While keeping the `eylah` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
```
You can go through each individual `json` file and manually remove the `eylah` field.
