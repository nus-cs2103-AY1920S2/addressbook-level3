...
/**
 * Changes the remark of an existing profile in the address book.
 */
...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the profile identified "
            + "by the index number used in the last profile listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the profile in the filtered profile list to edit the remark
     * @param remark of the profile to be updated to
     */
...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the profile's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a profile.

=== Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.profile`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let's go on a 5 minute adventure to get some placeholder text to show up for each profile.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.address.ui.DeadlineCard`].
...
.PersonCard.java
----
public PersonCard(Person profile, int displayedIndex) {
    //...
    remark.setText(profile.getRemark().value);
}
----
