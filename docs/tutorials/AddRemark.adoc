...
This command allows users of the AddressBook application to add optional remarks to people in their task list and edit it if required.
...
/**
 * Changes the remark of an existing person in the task list.
 */
...
Not doing so will cause AddressBook to default to an empty task list!
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedPerson = new Person(personToEdit.getName(), personToEdit.getPriority(), personToEdit.getEmail(),
