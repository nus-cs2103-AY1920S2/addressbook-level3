...
/**
 * Changes the remark of an existing coupon in the address book.
 */
...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the coupon identified "
            + "by the index number used in the last coupon listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the coupon in the filtered coupon list to edit the remark
     * @param remark of the coupon to be updated to
     */
...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the coupon's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a coupon.

=== Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.coupon`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let's go on a 5 minute adventure to get some placeholder text to show up for each coupon.
...
.PersonCard.java
----
public PersonCard(Person coupon, int displayedIndex) {
    //...
    remark.setText(coupon.getRemark().value);
}
----
