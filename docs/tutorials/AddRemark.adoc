...
We achieve that by working with the `Recipe` model.
Each field in a Recipe is implemented as a separate class (e.g. a `Name` object represents the recipe's name).
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.address.ui.RecipeCard`].
...
```
into link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-12580431f55d7880578aa4c16f249e71[`main/resources/view/RecipeListCard.fxml`].
...
We change the constructor of `Recipe` to take a `Remark`.
...
TIP: Use the `Find Usages` feature in IntelliJ IDEA on the `Recipe` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedRecipe` into `json` with the help of an external library -- Jackson.
Let's update `JsonAdaptedRecipe` to work with our new `Recipe`!
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Recipe`.
Since all fields in a `Recipe` are immutable, we create a new instance of a `Recipe` with the values that we want and save it with `Model#setRecipe()`.
