...
`seedu.zerotoone.ui.CommandBox.CommandExecutor`.
...
.Taking notes
====
Over the course of the debugging ongoingSession, you will encounter every major component in the application. Try to jot down
what happens inside the component and where the execution transfers to another component.
====

. To start the debugging ongoingSession, simply `Run` > `Debug Main`
...
+
.ExerciseListParser#parseCommand()
[source, java]
----
...
`JsonExerciseList#saveExerciseList()`.
. Again, it appears that the heavy lifting is delegated. Let's take a look at ``JsonSerializableExerciseList``'s
constructor.
+
.JsonSerializableExerciseList#JsonSerializableExerciseList()
[source, java]
----
/**
 ...
 * {@code JsonSerializableExerciseList}.
 */
...
this tutorial, you learned more about the inner workings of ExerciseList and how the various components mesh together
