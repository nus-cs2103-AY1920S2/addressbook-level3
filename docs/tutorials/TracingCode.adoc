...
command and store any possible changes in an `EditRecipeDescriptor`. Recall that we can verify the contents of
`editRecipeDesciptor` through the `Variable` tool window.
+
...
----
. It appears that a `JsonAdaptedRecipe` is created for each `Recipe` and then added to the
