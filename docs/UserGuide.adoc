...
NOTE: TA-Tracker data is saved periodically so you don’t have to worry about saving your data manually.
...
=== Session View
Under the sessions tab, the Session View contains the upcoming tasks/ undone sessions that you have.
...
Filter Student returns you all the students according to the given keywords below.

You can filter students in a class using these two ways:
...
1. Using both module code and group code returns you all the students specified by the group code and module code.
...
2. Using just the module code will return you all the students from the first group of module code shown in UI.
...
Filters all the students in module `CS2103T`, first group of CS2103T that is currently showing in UI.

[NOTE]
Keywords are case-insensitive. eg. `cs2103t` will match `CS2103T`
...
Adds a new session. The new undone session will be shown under Session view.

Format: `session add m/MOD_CODE [s/START] [e/END] [d/DATE] [w/WEEKS] [t/SESSION_TYPE] [n/NOTES]`

[TIP]
If you want to create a session at this point of time, you can leave the start time `s/START`, end time `e/END` and date `d/DATE` out. A session of current date and time will be automatically generated.

[NOTE]
====
[horizontal]
`START` `END` `DATE` `WEEK`:: see Common Parameters
...
Filters undone sessions fulfilling all provided constraints.

All undone sessions with matching keywords are displayed. Keyword is case-insensitive.
...
All undone sessions with module code `CS2103T` are displayed.

* `session filter m/CS2030T t/tutorial d/2020-03-20`
All undone sessions on `2020-03-20`, with module code  `CS2103T`, session type  ‘Tutorial` will be shown.
