...
as long as your date format follows a generic D/M/YY format (e.g. 11/4/20, 3-6-2019)!
...
In a similar way to `report`, `transactions` also allows you to see all the food you have purchased in
a particular period.
...
again or recommend it others? From the *menu screen* of a stall, you can easily set your favourite food items
...
painless to severe your ties, by allowing quick removal of that food.
...
* `TAG` - Label tag associated with the canteen or stall depending on which context you are on.
...
When filtering by _tag_, tags varies base on the *current context*.

****
Examples:
****
* If you wish to filter tags base on what the canteen has, you can enter
`*filter asian*` on the main screen. This will displays canteens with the asian tag.

* After entering a canteen, you can also filter the stalls base on tags as well.
