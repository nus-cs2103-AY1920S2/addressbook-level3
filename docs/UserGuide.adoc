...
This is because there is no `INDEX` specified, to correct this, you have to enter a valid `INDEX` after `delete-c`.

==== Index out of range
If you type in an `INDEX` that is out of range, that is if there are only 4 clients in the list but you type `delete-c 5`, no client will be deleted and an error message will be shown. 
