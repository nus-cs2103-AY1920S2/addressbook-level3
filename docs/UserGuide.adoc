...
====
If you have huge amount of orders or return orders to clear and tired of clearing them one by one, you could use the
`clear` command to remove all orders and return orders for just one command line.
====

==== How to use the Clear command?
* You can use this command to clear all existing orders and return orders in the order list and return order list respectively. By default, the command will ask you for confirmation.
...
* `clear` `[FLAG]` +
`clear` is the command and `FLAG` could be either `-f`, `-r` and `-o`; whereby `-f` flag indicate force clear and +
`-r` and `-o` indicates the return order list and order list respectively.

.

There are 6 possible combinations for the `clear` command:
...
====
Suppose you are given a huge amount of orders and return orders by your company and you are required to add them into
Delino. You would not want to add them one by one. +
No worries! Delino has the command, `import`, to import all of them at once via a CSV file.
====
...
. Possible combinations of Import command
|===
|Scenario |Command |Result

| If you want to import all of the orders and return orders at once via CSV file, you could use this:
| `import` `orders.csv`
