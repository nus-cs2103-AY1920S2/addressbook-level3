...
* `(ab)delete 2 t/` +
Deletes all the tags that belong to the 2nd person in the address book.
...
* [-g] indicates sorting by groups’ names.
...
Format: `(rt)addnote INDEX [r/RECOMMENDED_FOOD] [a/AWESOME_FOOD] [b/BAD_FOOD]`
...
==== [Coming Soon] Find restaurant : `(rt)find`

Finds restaurants based on a number of criteria. +
Format: `(rt)find [k/KEYWORD] [l/LOCATION] [p/PRICE_POINT] [o/OPERATING_HOURS]`
...
Example:
* `(st)schedule n/5` +
Shows the user's upcoming schedule for the next 5 days colour-coded based on the estimated work hours per day.
...
** `(rt)addnote INDEX [r/RECOMMENDED_FOOD] [a/AWESOME_FOOD] [b/BAD_FOOD]`
* *Delete notes* :
