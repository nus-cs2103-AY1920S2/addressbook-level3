...
Format: `add` `tid/*TRANSACTION_ID*` `n/*NAME*` `a/*ADDRESS*` `p/*PHONE_NUMBER*` `*ts/DELIVERY_DATE_&_TIME*` `w/*WAREHOUSE_LOCATION*` `pm/*PAYMENT_METHOD*` `[c/COMMENTS_BY_CUSTOMER]` `[type/TYPE_OF_ITEM]`

[TIP]
All the fields (in bold) are compulsory except for the fields [c/COMMENTS_BY_CUSTOMER] and [type/TYPE_OF_ITEM]
