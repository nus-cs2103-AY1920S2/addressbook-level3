...
* ratings you have given the student (on a scale from 1 - 5)
* and contact details such as email and phone number.
...
The claims under this tab can be sorted by module code to enable you to easily see
...
By using this command, you can switch to a specified tab.
...
By using this command, you can exit the program.

Format: `exit`

=== Student View

This section gives an explanation of all the commands that can be used
in the Student View.
...
The tab that you are viewing will automatically be switched to the student
...
setting. The tab that you are viewing will automatically be switched to the
...
code can't be changed. Editing a module doesn't affect the students and
...
setting. The tab that you are viewing will automatically be switched to the student
tab as well, if you were currently on a different tab.
...
the edited group belongs to and students of the edited group. The tab that you are viewing will automatically be switched to the student
tab as well, if you were currently on a different tab.
...
are given a default rating of 3 (average). You can either give them a custom rating in the add student
...
If there are no more students inside the module group, an empty list will
be shown.
...
You can use this command to sort the students belonging to a specified group.
The students will be sorted according to your specified `SORT_TYPE`.

When a group is sorted, Student View shows all the groups of the module that the sorted group
belongs to, as well as the students belonging to the group that has been sorted.
...
You can use this command to sort the students belonging to all the groups of the specified module.
The students will be sorted according to your specified `SORT_TYPE`.

When a module is sorted, Student View shows the groups of the module you specified,
as well as the students that belong to the first group in the module. If there are no groups in the module,
it will show an empty student list and module list.
...
You can sort all of the students in TA-Tracker.
The students will be sorted according to your specified `SORT_TYPE`.


When all modules are sorted, Student View shows the groups of the first module in the module list
and the students belonging to the first group of the module being shown. If there are no modules
...
The group with the specified `GROUP_CODE` and the module with the specified
`MOD_CODE` will be highlighted in the list.

Using both module code and group code will show you the students
inside the group with the given group code. The group must belong to the module specified
by the module code.
...
belonging to the first group of the module specified by the given module code.
...
The tab that you are viewing will automatically be switched to the session
tab as well, if you were currently on a different tab.
...
[TIP]
====
. If you want to create a session at the current point in time, you can omit the start time
`s/START`, end time `e/END` and date `d/DATE`.

. A session which has both the start time and the end time set to the current time (the time you entered the command into
TA-Tracker). will be automatically generated.

. Additionally, the date of the session will automatically set to be today's date.

. Once the session has ended, you can come back and edit the end time. You can also add other
...
====
[horizontal]
`NOTES`:: notes for this session (e.g.: weekly tutorials, prep sessions, consultation with John Doe)
====
...
from 2pm to 4pm,
containing a note "with Alice and Bob".

====
==== Deleting a Session : `session delete`

You can use this command to delete a session.
...
Deletes the session with the given unique index from TA-Tracker.
...
** The new start time at 14:00
** The new end time at 16:00
** The new date on 19-02-2020
** The new module code CS3243
...
The tab that you are viewing will automatically be switched to the claims
tab as well, if you were currently on a different tab.
...
You should be specific when filtering sessions. You can filter the sessions by using the following
keywords:
...
The module with the specified `MOD_CODE` will be highlighted in the list.
