...
NOTE: This has to be a *positive numerical number* (decimal places allowed).
...
This command generate a report for the spendings and food purchases for any period specified. +
Formats: `report` +
...
NOTE: `START_DATE` cannot be a future date. +
...
TIP: User  can also update existing reviews by using the same command.
