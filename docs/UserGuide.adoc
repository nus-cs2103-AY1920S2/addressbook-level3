...
[TIP]
====
. If you want to create a session at the current point in time, you can leave the start time
`s/START`, end time `e/END` and date `d/DATE` out. This will create a session which has both
the start time and the end time set to the current time (the time you entered the command into
TA-Tracker).
