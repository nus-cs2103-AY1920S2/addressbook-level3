...
In addition, the reminder entry list will also be sorted in the same manner specified by the user.
...
* `sort x/f by/time o/a` (sort food entry list and reminder list in ascending order of recording time,
i.e. the older entry comes first)
* `sort by/c` (sort both food and sports entry list, as well as reminder list,
