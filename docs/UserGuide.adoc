...
+
image::JarExample.png[width="790"]
This image shows how you can identify what the latest release is and what file
you should download.
+
...
The purpose of this view is to *allow you to keep track of all your claims* so
...
* `MOD_CODE` here refers to the module code of the module that contains the group
you want to delete.
...
command or,
* you can edit their rating later by using the `student edit` command (details given later in
this guide).
====
//tag::endstudent[]
//tag::deletestudent[]
...
command (details given later in this guide).
If you were on a different `tab`, you will automatically be switched to the `student tab`.
...
. You can add sessions with time clashes so that you can quickly plan a session
and correct the details later.
====
...
* `SESSION_TYPE` - filtering by session type will show you only the
sessions with the *specified type*.
...
for the command `session filter m/CS3243 t/grading` you will be shown sessions that
either belong to the module `CS3243` or are of type `grading`.
...
All sessions in the *Session View* with module code `CS3243`,
*or* session type `Grading` will be shown.
====

[NOTE]
====
As long as a session contains *any one of the keywords* given by the user,
the session will be shown to the user.
====
//end::filtersession[]
...
* `RATE` is the amount you want to change the hourly rate to, this value will be used to calculate the
...
Optionally, you can specify a *module code*. If a module code is specified,
the report generated will only include data from the *specified module*.
...
In V2.0, you will be able to *add timed sessions*. Timed sessions will allow you
to set sessions that will *notify
and remind* you as the sessions' start time approaches.
Timed sessions will also allow you to *start and stop sessions
