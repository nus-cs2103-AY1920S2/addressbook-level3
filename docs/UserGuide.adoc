...
`mode` to switch the dieting mode, based on the following modes: +
...
[TIP]
====
Switch the mode to help yourself reach your diet targets better!
====
...
[WARNING]
You *MUST* use `list` to check the lists of items to identify a target to edit. Failure to do so might cause unintended consequences to the Diet Tracker's Food List.
...
`metrics` You can use the `help` command, if you are uncertain, to see the list of available commands for Diet Tracker.
...
* *Listing Receipt* `listreceipt` +
e.g. `listreceipt`

* *Listing Amount* `listamount` +
e.g `listamount`

* *Marking Receipt as Done* `donereceipt` +
e.g `donereceipt`

* *Getting Paid by Someone* `paid INDEX` +
e.g. `paid 3`
