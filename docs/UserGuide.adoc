...
* If you accidentally added an order or a return order and realise that it have a typo in any of it's field, fear not! The `Edit` function is here to save you.
* To find out more about how to properly use the `edit` feature, scroll to section <<edit-works, How to use the Edit command>> and <<edit-examples, What constitutes a valid Edit command>>!
...
*Step 2* : Provide the `<<command_flag, FLAG>>` corresponding to the parcel order type you want to edit +
...
[NOTE]
====
* `edit` is the command for the edit feature.
...
====
* Having trouble finding specific parcels in the list? Tired of scrolling through the whole list of orders? +
It's okay! Delino provide you with the `search` feature to ease the tedious process of finding a particular parcel +
With the `search` feature you can now find a parcel instantly! +
Find out how to use it in the  <<search-works, How to use the Search command>> and <<search-examples, What constitutes a valid Search command>> section!
...
[NOTE]
====
If no flag is given in this step, search feature will be performed on both the order and return list.
====
