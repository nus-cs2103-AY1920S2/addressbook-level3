...
* `INDEX`: Enter canteen by listed index.

* `CANTEEN_NAME`: Enter canteen by its name.

NOTE: The display of the store rating is determined from the user's past experiences of the food items which were selected.
...
* `INDEX`: Select by the food's index on the list.
* `FOOD_NAME`: Select by name of the food.

NOTE: If a budget has been set up, the budget will be automatically
reduce by the price of the selected food.
...
* `[c/ CANTEEN_NAME]` - Randomize based on the canteen selected.
** Example: `randomize c/ The Deck`
* `[t/ TAG]` - Randomize based on the tag provided.
...
as well as the remaining available budget to spend.

Format: `budget view`

=== View Expense Report: `*report*`

Generates a report for your spending and food purchases for any period specified.
...
** Example: `report [m/ jan]` - Generates a report for the month of the current year.
** Example: transactions `[m/ jul] [y/ 2019]` - Generates a report for the month of the specified year.
...
You can review food items from the transactions screen as shown in 3.12.
...
You can rate food items from the transactions screen as shown in 3.12.
