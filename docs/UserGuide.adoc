...
* *Done* : `done` `INDEX` +
e.g. `done` `2`
* *Edit* : `edit` `INDEX` `FIELD_TO_CHANGE` `CHANGES` +
e.g. `edit` `2` `name` `Eng Xuan En`
* *Exit* : `exit`
...
e.g. `list` `done`
* *Return* : `return` `tid/TRANSACTION_ID` `n/NAME` `a/ADDRESS` `p/PHONE_NUMBER` `ts/DELIVERY_DATE_&_TIME` `w/WAREHOUSE_LOCATION` `pm/PAYMENT_METHOD` `c/COMMENTS_BY_CUSTOMER` `type/TYPE_OF_ITEM` +
