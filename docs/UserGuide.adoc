...
[[search]]
=== Locating orders by name or transaction ID: `search`

==== Description

* You can search all orders that contain a given keyword. The orders will be searched by a given flag of two specific types (`-n` or `-t`).
...
==== Format
* `search` `FLAG` `KEYWORD` `[MORE_KEYWORDS]...`
...
==== Examples:
* `return` `tid/ac17s2a` `n/Bobby Tan` `a/123 Delta Road #03-333, Singapore 123456` `p/91230456` `rts/12-12-2020 1300` `w/Jurong Warehouse` `c/NIL` `type/glass` +
Adds the order with transaction id 'ac17s2a' as a return parcel into the list of returns.

* `return` `tid/b1230512` `n/Aaron Teo` `a/256 Alpha Road #03-222, Singapore 123567` `p/91230456` `rts/12-12-2020 1400` `w/Jurong Warehouse` `c/Leave it at the lobby` `type/metal` +
...
You can obtain more information about each area from
https://keylocation.sg/singapore/districts-map[here] +
====
* You can view the orders located at a specified *area*.


More information about the 5 areas of Singapore are shown below: +
...
* <<exit, *Exit*>> : `exit`
* <<search, *Search*>> : `search` `flag` `KEYWORD` +
e.g. `search` `-t` `ac1e345x7s`
* <<help, *Help*>> : `help`
* <<import, *Import*>> : `import` `FILE_NAME` +
e.g. `import` `orders.csv`
* <<list, *List*>> : `list` `[DONE_STATUS]` +
e.g. `list` `done`
* <<return, *Return*>> : `return` `tid/TRANSACTION_ID` `n/CUSTOMER_NAME` `a/ADDRESS` `p/PHONE_NUMBER` `rts/RETURN_DATE_&_TIME` `w/WAREHOUSE_LOCATION` `c/COMMENTS_BY_CUSTOMER` `type/TYPE_OF_ITEM` +
