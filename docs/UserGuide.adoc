...
* Items in square brackets are optional e.g both `open -t Notably` and `open Notably` will open the note titled "Notably".
...
If you want to change the `BODY` content of a note, use the `edit` command. This command *updates* the existing `BODY` content of the currently open note, with the new `BODY` content. +
...
. Let's update the `BODY` content of the currently opened note `Workspace`, with "This is the new paragraph". This can be done with the command: +
...
=== (Coming in v2.0) Find a note based on certain keywords: `search`
...
* *(Coming in v2.0) Find a note based on certain keywords*: `search [-s] KEYWORD`
