...
`Find-c` allows you to find clients with the names that are specified in your input. You might have many clients and finding a particular client by scrolling through the entire list might be troublesome and difficult.

==== Parameters


* `NAME` is case insensitive. e.g `hans` will match `Hans`
...
You might wonder why `find-c` do not show the clients even though the client is in clearly in your client list. It might be possible that you have typed an incomplete name and does not match any word in the name of that client. The example below might help you understand better.  
...
This is because the name that you have entered is incomplete and does not match any word in the name of any of the clients. To correct this, you have to type `find-c bryan` in this case.

/photo

===== Using multiple words as parameters
