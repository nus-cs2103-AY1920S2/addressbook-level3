...
* If you use a prefix without `...` mentioned multiple times, TA-Tracker will accept
the last instance of the prefix.
For example, in the command `module add m/CS2103T n/SE n/Software Engineering` ,
a module with the module code `CS2013T` and name `Software Engineering` will be created.

* If you separate the the prefix letter and `/` while writing a command, the TA-Tracker
will not recognise the prefix. For example, in the command `module add m/CS2103T
n /Software Engineering` , the TA-Tracker doesn't recognise `n /` to be the same
as `n/` and you'll get an error message saying that the command format used it wrong.
...
what claims to submit for each of the modules you teach.
...
[TIP]
====
. If you want to create a session with the current date, and start time but aren't
sure when the session will end, you can omit `e/END` and `d/DATE`.
