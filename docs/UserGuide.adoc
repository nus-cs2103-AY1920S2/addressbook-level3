...
[[nearby]]
=== View orders located at the same area or postal sector: `nearby` [Done by - Loh Jeremy Wen Jun]
====
...
** With the `nearby` command, you can identify all such orders by providing either
a region or postal sector!
====
...
*Step 2 (optional)* : Provide an optional `<<command_flag, FLAG>>` that indicates which order list to
search (order list or return order list) you want to edit. +
[NOTE]
====
...
The table below describes the possible usage combinations of the Nearby command:
...
|If you want to obtain all orders in the order list located in the central region of Singapore.
|`nearby` `-o` `central`
