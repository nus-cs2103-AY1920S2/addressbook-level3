...
You can mark an existing recipe from *CookBuddy* as being attempted using the `done` command.

Format: `done INDEX`

Example: You can type `done 1` and *CookBuddy* will mark the recipe at index 1 as having been attempted.


=== Un-Marking a recipe as done — `undo`
You can un-mark an existing recipe from *CookBuddy* as being done, indicating that it has not been attempted, using the
`undo` command.
