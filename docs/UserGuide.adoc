...
[caption=]
.1) If you would like to add a `recipe` to your CookBuddy, enter the `new` command including the specific metadata of the `recipe`
image::user-guide/new-before.png[width="300"]

[caption=]
.2) After using the `new` command, the new `recipe` will be added to your CookBuddy and will be displayed.
...
[caption=]
.1) You want to view the second recipe in your CookBuddy
...
[caption=]
.1) You want to delete the first recipe in your CookBuddy
image::user-guide/delete-before.png[width="600"]

[caption=]
.2) After using the `delete` command, the recipe will be removed from your CookBuddy
image::user-guide/delete-after.png[width="300"]

[IMPORTANT]
====
This command cannot be undone. Once a `recipe` has been deleted, its respective data folder will be permanently deleted,
together with all its associated data, including its `images`. +
...
[caption=]
.1) You want to modify the `recipe tags` in your CookBuddy
...
[caption=]
.1) You want to modify (remove) all the `tags of a recipe` in your CookBuddy
image::user-guide/modifyremovetag-before.png[width="600"]

[caption=]
.2) After using the `modify` command, the `recipe tags` will be removed accordingly
