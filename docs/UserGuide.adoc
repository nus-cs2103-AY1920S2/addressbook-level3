...
If you entered `find-c bryan low`,
...
==== Parameters
This section acts as a summary of the important things to note when using `add-e` including their parameters.
...
| Parameters | Important points to note
| n/NAME |
* `n/NAME` is compulsory and you have to be specified it.

| d/DATE |
* `d/DATE` is compulsory and you have to be specified it.
...
==== Example
Let's say that you want to find a client named "Bryan Low" in client list. You can simply use the `add-e` command by following the steps below.

1.View the client that you want to add the exercise to. For information on how to view the client, you can refer to section on `...:view-c`. For this example, we will add an exercise to the first client in the list. After you entered `view-c 1`,
