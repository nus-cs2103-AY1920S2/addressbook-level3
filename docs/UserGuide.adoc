...
=== Common Symbols (Contributed by Fatin)
...
=== Command Format (Contributed by Fatin and Gabriel)
...
==== Viewing help : `help` (Contributed by Fatin)
...
==== Switching tabs : `goto` (Contributed by Fatin)
...
==== Marking a Session as Done : `session done` (Contributed by Fatin)
...
* `session filter m/CS2030T t/tutorial`
+
...
+
All sessions in the *Session View* that take place either on `2020-03-20`, *or* with module code `CS2103T`,
*or* session type `Tutorial` will be shown.
====
...
[#setrate]
==== Changing the hourly rate : `setrate` (Contributed by Fatin)
...
[NOTE]
====
* If you don't specify a rate, it is set at $40.00 by default (the rate at which most SOC TAs are being paid per hour).
