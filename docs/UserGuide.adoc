...
Find customers whose names contain any of the given keywords, and whose attributes match the given attributes. +
...
Add a transaction with product 1 and customer 10 of 20 dollars at 2020-02-19 19:00.
* `addt p/20 c/2 dt/2020-02-20 10:00 m/30 d/under discount` +
Add a transaction with product 20 and customer 2 of 30 dollars at 2020-02-20 10:00.
...
Find transactions whose attributes match the given attributes. +
...
=== View the top-selling and worst-selling product.

Dashboard that will display and updates the top-selling and worst-selling product as transactions are made. +
Example of dashboard that will display and update top selling products with the revenue it brings in. +

Example:

image::TopSellingProduct.png[]

=== View the revenue trend in a certain period : `revenue`
...
=== Set the low-inventory threshold : `lowinv`

Format: `lowinv [p/PRODUCT_ID] t/THRESHOLD`

****
* THRESHOLD must be in numerical format.
...
* `lowinv p/001 t/20` +
Set the low inventory threshold for product 001 as 20.

=== Receive notification while a certain product is running low.

This is a passive feature. If any product’s stock is running low, the user will receive a warning message after opening the application.

=== Predict the sales for the next month : `predict`.

Plot a line graph representing the predicted sales for each day in the next month. +
Format: `predict`
...
* *Find products* : `findp [k/KEYWORD] [pr/LOW-HIGH] [s/LOW-HIGH] [q/LOW-HIGH]` +
...
* *Clearing all transactions* : `cleart`
* *View the revenue trend in a certain period* : `revenue`
* *Set the low-inventory threshold* : `lowinv [p/PRODUCT_ID] t/THRESHOLD` +
