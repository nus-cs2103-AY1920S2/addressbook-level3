...
=== Listing all suppliers : `list-s` (Liu Chao)

Shows a list of all suppliers in the inventory manager. +
Format: `list-s`

// end::delete[]
=== Clearing all supplier entries : `clear-s` (Liu Chao)

Clears all supplier entries from the address book. +
Format: `clear-s`

=== Adding a supplier: `add-s` (Liu Chao)
...
=== Editing a supplier : `edit-s` (Liu Chao)
...
=== Locating suppliers by name: `find-s` (Liu Chao)
...
=== Source supplier(s) for good(s): `source` (Liu Chao)
...
// tag::delete[]
=== Deleting a supplier : `delete-s` (Liu Chao)
...
=== Deleting an entry or entries of good-price pair(s) in the supplier's list: `delete-gp` (Liu Chao)
