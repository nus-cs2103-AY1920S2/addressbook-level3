...
In this section, we look at one of the key features of Coupon Stash's
Graphical User Interface. The interface can be broadly split into
3 different parts based on the different tabs present in the
program, namely Coupons, Summary and Help. Also, the calendar and
command result box are two features visible on every tab.
...
=== Coupons tab
The coupons tab contains a visual overview of the coupons which
you have added to the application.
...
[WARNING]
As compared to the User Guide, the description in the
help tab is less detailed!
