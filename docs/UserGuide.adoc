...
Format: `edit INDEX [n/NAME] [e/EXPIRY_DATE] [s/SAVINGS_OR_FREE_ITEM] [sd/START_DATE] [r/REMIND_DATE] [p/PROMO_CODE] [c/CONDITION] [l/USAGE_LIMIT] [s/EXTRA_FREE_ITEMS]... [t/TAG]...`
...
[NOTE]
The `expiring` command will include archived coupons in its results
too. Coupons that are expiring in the archives are
...
archived coupons , if they are present on screen. Archived coupons will always appear
...
pressing the keyboard buttons kbd:[Ctrl] and kbd:[q] simultaneously.
