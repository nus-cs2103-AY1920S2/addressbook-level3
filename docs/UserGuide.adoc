...
* If you have yet to decided on a focus area, you can choose to add it to your profile using the <<Edit, edit>> command. Other profile fields can be changed similarly.
* Only one profile is allowed. If you wish to create a new profile, please remove it with the <<Delete,`delete`>> feature mentioned in <<Delete, Section 4.6>>
* The course / focus name entered has to be its full name. Please refer to the <<Appendix, Appendix>> for the list of courses and focus areas supported
* `cs/CURRENT_SEMESTER` must be entered as an integer, e.g. if you are currently in Year 2 Semester 1, you should enter `cs/3`
...
NOTE: If you request for MODdy to show multiple information at one time, such as course information on Computer Science and module information on CS1101S, using the command `show c/Computer Science m/1101s`, only 1 object will be shown. +
The object that is prioritised to be shown is in this order, beginning with the highest priority: Profile, Semester information, Module information, Focus area information, Course information.
// end::show[]
